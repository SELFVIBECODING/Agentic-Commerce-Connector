...
#   2. Detects existing install → bails cleanly to avoid clobbering state.
...
#   7. Runs `acc init shopify` interactively (re-attaching stdin to /dev/tty
#      so the wizard works even when piped through `curl | bash`).
...
echo
echo "▲  ACC server bootstrap (Shopify)"
...
Documentation=https://github.com/SELFVIBECODING/Agentic-Commerce-Connector
...
# ── 5. Interactive acc init shopify ────────────────────────────────────────

cat <<EOF

──────────────────────────────────────────────────────────────────
Now running \`acc init shopify\` — the Shopify-connect wizard.

You'll be prompted for:
  • public URL          → auto-filled from ACC_PUBLIC_HOSTNAME
  • encryption key      → auto-generated on first run
  • signer wallet       → auto-generate (recommended) or import
  • Shopify Partners creds (client_id + client_secret from your app)

Before continuing, ensure you have created a Shopify Partners app
following https://www.siliconretail.com/docs/shopify-partners-setup
(takes ~10 min; one-time; standalone of this installer).
──────────────────────────────────────────────────────────────────
...
# Seed the public URL; everything else is interactive. Invoke `acc init
# shopify` explicitly so the platform menu is skipped in the scripted flow.
sudo -u "${ACC_USER}" \
     --preserve-env=ACC_PUBLIC_HOSTNAME \
     -- bash -lc "cd ~ && ACC_PUBLIC_HOSTNAME='${ACC_PUBLIC_HOSTNAME}' /home/${ACC_USER}/.acc/bin/acc init shopify"
