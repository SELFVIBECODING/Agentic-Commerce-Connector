...
**Re-running:** `acc init shopify` detects an existing `config.json` and offers: keep-as-is / re-run Shopify install (switch method or rotate creds) / start-over (backs up old config) / cancel.
